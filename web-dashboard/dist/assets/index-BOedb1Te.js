...
(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const o of i)if(o.type==="childList")for(const a of o.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&n(a)}).observe(document,{childList:!0,subtree:!0});function t(i){const o={};return i.integrity&&(o.integrity=i.integrity),i.referrerPolicy&&(o.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?o.credentials="include":i.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(i){if(i.ep)return;i.ep=!0;const o=t(i);fetch(i.href,o)}})();const be={};let U=null;function A(e,s){be[e]=s}function De(e="#/login"){async function s(){const t=window.location.hash||e,n=sessionStorage.getItem("hive_user");if(!n&&t!=="#/login"){window.location.hash="#/login";return}if(n&&t==="#/login"){window.location.hash="#/apiary";return}let i=null,o={};const a=t.split("?")[0];for(const[l,d]of Object.entries(be)){const c=l.split("/"),p=a.split("/");if(c.length!==p.length)continue;let v=!0;const h={};for(let m=0;m<c.length;m++)if(c[m].startsWith(":"))h[c[m].slice(1)]=decodeURIComponent(p[m]);else if(c[m]!==p[m]){v=!1;break}if(v){i=d,o=h;break}}if(i){U&&(U(),U=null);const l=document.getElementById("app"),d=await i(l,o);typeof d=="function"&&(U=d)}}window.addEventListener("hashchange",s),s()}const ye="apiary_theme";function qe(){localStorage.getItem(ye)==="light"&&document.documentElement.classList.add("light"),ae()}function je(){document.documentElement.classList.toggle("light");const e=document.documentElement.classList.contains("light");localStorage.setItem(ye,e?"light":"dark"),ae()}function ae(){const e=document.documentElement.classList.contains("light");document.querySelectorAll(".theme-icon-dark").forEach(s=>s.classList.toggle("hidden",e)),document.querySelectorAll(".theme-icon-light").forEach(s=>s.classList.toggle("hidden",!e)),document.querySelectorAll(".theme-knob").forEach(s=>{s.style.left=e?"14px":"2px"})}function we(){document.querySelectorAll("#themeToggle").forEach(e=>{const s=e.cloneNode(!0);e.parentNode.replaceChild(s,e),s.addEventListener("click",()=>{je()})}),ae()}function V(e,s,t){const n=[];for(let i=0;i<6;i++){const o=Math.PI/3*i-Math.PI/6;n.push(`${e+t*Math.cos(o)},${s+t*Math.sin(o)}`)}return`M${n.join("L")}Z`}function Re(){const o=[];for(let a=0;a<5;a++){const l=a%2===0?4:3;for(let d=0;d<l;d++){const c=d*140.4+(a%2===0?0:105.30000000000001)+78,p=650-a*156*.5-78,v=Math.sqrt(c*c+(650-p)*(650-p))/Math.sqrt(750*750+650*650),h=Math.max(0,1-v*2);h>.05&&o.push({x:c,y:p,opacity:h})}}return ke(750,650,o,78,"bl")}function Pe(){const o=[];for(let a=0;a<4;a++)for(let d=0;d<3;d++){const c=600-d*144-80,p=a*160*.5+80,v=Math.sqrt((600-c)*(600-c)+p*p)/Math.sqrt(600*600+500*500),h=Math.max(0,1-v*2.2);h>.05&&o.push({x:c,y:p,opacity:h})}return ke(600,500,o,80,"tr")}function ke(e,s,t,n,i){const o=document.documentElement.classList.contains("light"),a="http://www.w3.org/2000/svg",l=document.createElementNS(a,"svg");l.setAttribute("width","100%"),l.setAttribute("height","100%"),l.setAttribute("viewBox",`0 0 ${e} ${s}`),l.setAttribute("preserveAspectRatio","none");const d=document.createElementNS(a,"defs"),c=document.createElementNS(a,"filter");c.setAttribute("id",`emboss-${i}`),c.setAttribute("x","-20%"),c.setAttribute("y","-20%"),c.setAttribute("width","140%"),c.setAttribute("height","140%");const p=document.createElementNS(a,"feGaussianBlur");p.setAttribute("in","SourceAlpha"),p.setAttribute("stdDeviation","3"),p.setAttribute("result","blur");const v=document.createElementNS(a,"feDiffuseLighting");v.setAttribute("in","blur"),v.setAttribute("surfaceScale","6"),v.setAttribute("diffuseConstant","0.7"),v.setAttribute("result","diffuse"),v.setAttribute("lighting-color",o?"#c8b890":"#4a6a8a");const h=document.createElementNS(a,"feDistantLight");h.setAttribute("azimuth","315"),h.setAttribute("elevation","25"),v.appendChild(h);const m=document.createElementNS(a,"feSpecularLighting");m.setAttribute("in","blur"),m.setAttribute("surfaceScale","5"),m.setAttribute("specularConstant","0.4"),m.setAttribute("specularExponent","20"),m.setAttribute("result","specular"),m.setAttribute("lighting-color",o?"#e8dcc0":"#8ab4d8");const r=document.createElementNS(a,"feDistantLight");r.setAttribute("azimuth","315"),r.setAttribute("elevation","30"),m.appendChild(r);const u=document.createElementNS(a,"feComposite");u.setAttribute("in","specular"),u.setAttribute("in2","diffuse"),u.setAttribute("operator","arithmetic"),u.setAttribute("k1","0"),u.setAttribute("k2","1"),u.setAttribute("k3","1"),u.setAttribute("k4","0"),u.setAttribute("result","lit");const f=document.createElementNS(a,"feComposite");f.setAttribute("in","lit"),f.setAttribute("in2","SourceAlpha"),f.setAttribute("operator","in"),f.setAttribute("result","clipped");const y=document.createElementNS(a,"feMerge"),w=document.createElementNS(a,"feMergeNode");w.setAttribute("in","clipped");const $=document.createElementNS(a,"feMergeNode");$.setAttribute("in","SourceGraphic"),y.appendChild(w),y.appendChild($),c.appendChild(p),c.appendChild(v),c.appendChild(m),c.appendChild(u),c.appendChild(f),c.appendChild(y),d.appendChild(c);const S=document.createElementNS(a,"filter");S.setAttribute("id",`shadow-${i}`),S.setAttribute("x","-30%"),S.setAttribute("y","-30%"),S.setAttribute("width","160%"),S.setAttribute("height","160%");const I=document.createElementNS(a,"feDropShadow");return I.setAttribute("dx","4"),I.setAttribute("dy","6"),I.setAttribute("stdDeviation","8"),I.setAttribute("flood-color",o?"rgba(0,0,0,0.15)":"rgba(0,0,0,0.5)"),S.appendChild(I),d.appendChild(S),l.appendChild(d),t.forEach(({x:N,y:M,opacity:g})=>{const E=document.createElementNS(a,"g");E.setAttribute("opacity",String(g));const x=document.createElementNS(a,"path");x.setAttribute("d",V(N,M,n*.92)),x.setAttribute("fill",o?"rgba(180,165,140,0.3)":"rgba(8,16,28,0.7)"),x.setAttribute("filter",`url(#shadow-${i})`),E.appendChild(x);const b=document.createElementNS(a,"path");b.setAttribute("d",V(N,M,n*.92)),b.setAttribute("fill",o?"rgba(215,205,185,0.6)":"rgba(14,28,48,0.85)"),b.setAttribute("filter",`url(#emboss-${i})`),E.appendChild(b);const L=document.createElementNS(a,"path");L.setAttribute("d",V(N,M,n*.92)),L.setAttribute("fill","none"),L.setAttribute("stroke",o?"rgba(255,255,255,0.4)":"rgba(255,255,255,0.06)"),L.setAttribute("stroke-width","1.5"),E.appendChild(L);const T=document.createElementNS(a,"path");T.setAttribute("d",V(N,M,n*.95)),T.setAttribute("fill","none"),T.setAttribute("stroke",o?"rgba(160,145,120,0.2)":"rgba(255,255,255,0.04)"),T.setAttribute("stroke-width","0.8"),E.appendChild(T),l.appendChild(E)}),l}function Oe(){de(),new MutationObserver(()=>{document.querySelectorAll(".hex-corner-svg").forEach(s=>s.remove()),de()}).observe(document.documentElement,{attributes:!0,attributeFilter:["class"]})}function de(){const e=document.createElement("div");e.className="hex-corner-svg hex-corner-svg--bl",e.setAttribute("aria-hidden","true"),e.appendChild(Re()),document.body.insertBefore(e,document.body.firstChild);const s=document.createElement("div");s.className="hex-corner-svg hex-corner-svg--tr",s.setAttribute("aria-hidden","true"),s.appendChild(Pe()),document.body.insertBefore(s,document.body.firstChild)}const Fe="https://093c0f3bca66ea3daf04bcc8a98c06.10.environment.api.powerplatform.com:443/powerautomate/automations/direct/workflows/64df8c23e1f548e9a7888164706e316c/triggers/manual/paths/invoke?api-version=1&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=EeX8QCqqiPt7LsuoR44C2Cah6jwu_6jBcT0901ePSd8",ce="%%APIARY_READ_FLOW_URL%%",ue="%%APIARY_WRITE_FLOW_URL%%";let B=[],P=[],_=[],F=[],Z=!1;async function We(){if(!Z)try{const[e,s,t,n]=await Promise.all([Y("hives"),Y("inspections"),Y("notes"),Y("tasks")]);B=e.map(Ue),P=s.map(Ve).sort((i,o)=>new Date(o.date)-new Date(i.date)),_=t.map(Ye),F=n.map(Ge),Z=!0}catch(e){console.error("Failed to load from Dataverse:",e),Z=!0}}async function Y(e){if(ce.includes("%%"))return console.warn(`Read flow not configured for ${e}`),[];const s=new URL(ce);s.searchParams.set("entity",e);const t=await fetch(s.toString());if(!t.ok)throw new Error(`Read ${e}: HTTP ${t.status}`);return t.json()}async function ze(e,s,t,n){if(ue.includes("%%"))return console.warn("Write flow not configured"),null;const i=await fetch(ue,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({entity:e,operation:s,data:t,id:n})});if(!i.ok)throw new Error(`Write ${e}/${s}: HTTP ${i.status}`);const o=await i.text();return o?JSON.parse(o):null}function k(e,s,t,n){ze(e,s,t,n).catch(i=>console.error("Async write failed:",i))}function ee(e,s){if(!e)return s;try{return JSON.parse(e)}catch{return s}}function Ue(e){return{id:e.gr_hiveid,hiveName:e.gr_name,type:e.gr_hivetype||"Hive",hiveStyle:e.gr_hivestyle||"",status:e.gr_status||"Active",strength:e.gr_strength??0,beeType:e.gr_beetype||"",color:e.gr_color||"#f59e0b",queenMarked:!!e.gr_queenmarked,queenColor:e.gr_queencolor||null,queenYear:e.gr_queenyear||null,queenClipped:!!e.gr_queenclipped,queenSource:e.gr_queensource||"",queenAddedDate:e.gr_queenaddeddate?e.gr_queenaddeddate.slice(0,10):null,queenNotes:e.gr_queennotes||"",dateAdded:e.gr_dateadded?e.gr_dateadded.slice(0,10):new Date().toISOString().slice(0,10),orientation:e.gr_orientation||"vertical",components:ee(e.gr_components,[])}}function W(e){return{gr_name:e.hiveName,gr_hivetype:e.type,gr_hivestyle:e.hiveStyle,gr_status:e.status,gr_strength:e.strength,gr_beetype:e.beeType,gr_color:e.color,gr_queenmarked:e.queenMarked,gr_queencolor:e.queenColor,gr_queenyear:e.queenYear,gr_queenclipped:e.queenClipped,gr_queensource:e.queenSource,gr_queenaddeddate:e.queenAddedDate,gr_queennotes:e.queenNotes,gr_components:JSON.stringify(e.components||[]),gr_dateadded:e.dateAdded,gr_orientation:e.orientation}}function Ve(e){return{id:e.gr_inspectionid||e.gr_name,date:e.gr_activitydate?e.gr_activitydate.slice(0,10):"",type:e.gr_activitytype||"Inspection",hive:e.gr_hivename||"",strength:e.gr_strength,queenSeen:!!e.gr_queenseen,broodSpotted:!!e.gr_broodspotted,queenCells:!!e.gr_queencells,temperament:e.gr_temperament||"",broodPattern:e.gr_broodpattern||"",weightLeft:e.gr_weightleft,weightRight:e.gr_weightright,weightTotal:e.gr_weighttotal,diseases:ee(e.gr_diseases,[]),pests:ee(e.gr_pests,[]),notes:e.gr_notes||"",weatherTemp:e.gr_weathertemp,weatherConditions:e.gr_weatherconditions||"",_dvId:e.gr_inspectionid}}function $e(e){return{gr_name:e.id||"insp-"+Date.now(),gr_activitydate:e.date,gr_activitytype:e.type||"Inspection",gr_hivename:e.hive,gr_strength:e.strength,gr_queenseen:e.queenSeen,gr_broodspotted:e.broodSpotted,gr_queencells:e.queenCells,gr_temperament:e.temperament,gr_broodpattern:e.broodPattern,gr_weightleft:e.weightLeft,gr_weightright:e.weightRight,gr_weighttotal:e.weightTotal,gr_diseases:JSON.stringify(e.diseases||[]),gr_pests:JSON.stringify(e.pests||[]),gr_notes:e.notes,gr_weathertemp:e.weatherTemp,gr_weatherconditions:e.weatherConditions}}function Ye(e){return{id:e.gr_noteid||e.gr_name,text:e.gr_text||"",date:e.gr_notedate?e.gr_notedate.slice(0,10):"",pinned:!!e.gr_pinned,deleted:!!e.gr_deleted,hiveId:e.gr_hiveid||null,_dvId:e.gr_noteid}}function R(e){return{gr_name:e.id||"n"+Date.now(),gr_text:e.text,gr_notedate:e.date,gr_pinned:e.pinned,gr_deleted:e.deleted,gr_hiveid:e.hiveId||""}}function Ge(e){return{id:e.gr_taskid||e.gr_name,text:e.gr_text||"",done:!!e.gr_done,due:e.gr_duedate?e.gr_duedate.slice(0,10):null,deleted:!!e.gr_deleted,_dvId:e.gr_taskid}}function oe(e){return{gr_name:e.id||"t"+Date.now(),gr_text:e.text,gr_done:e.done,gr_duedate:e.due,gr_deleted:e.deleted}}async function Qe(){const e=await fetch(Fe);if(!e.ok)throw new Error(`Telemetry fetch failed: HTTP ${e.status}`);return(await e.json()).map(t=>({weight:t.gr_weight,internalTemp:t.gr_internaltemp,batteryVoltage:t.gr_batteryvoltage,hiveHum:t.gr_hivehum,legTemp:t.gr_legtemp,timestamp:t.gr_readingtimestamp||t.createdon})).sort((t,n)=>new Date(t.timestamp)-new Date(n.timestamp))}const D={name:"Home Apiary",location:"Kidmore End, Reading RG4 9AY, UK",lat:51.509,lng:-.975};function Se(){return P}function Je(){const e={};for(const s of P)e[s.date]||(e[s.date]=[]),e[s.date].push(s);return Object.entries(e).sort(([s],[t])=>new Date(t)-new Date(s)).map(([s,t])=>({date:s,items:t,count:t.length}))}function Ke(e){return P.filter(s=>s.hive===e)}function Ce(){return[]}function Ze(e){const s={...e,id:e.id||"insp-"+Date.now()};P.unshift(s),k("inspections","create",$e(s))}const te=[{id:"hive-roof",name:"Roof",color:"#7a8078",height:14,category:"structure",nuc:!1},{id:"hive-floor",name:"Floor",color:"#8B8580",height:10,category:"structure",nuc:!1},{id:"hive-stand",name:"Hive Stand",color:"#A09080",height:22,category:"structure",nuc:!1},{id:"super",name:"Super",color:"#90968b",height:30,category:"box",nuc:!1},{id:"national-brood",name:"National Brood",color:"#90968b",height:45,category:"box",nuc:!1},{id:"14x12-brood",name:"14x12 Brood",color:"#90968b",height:63,category:"box",nuc:!1},{id:"queen-excluder",name:"Queen Excluder",color:"#BFA67A",height:6,category:"accessory",nuc:!1},{id:"hive-eke",name:"Hive Eke",color:"#A89068",height:18,category:"accessory",nuc:!1},{id:"nuc-roof",name:"Nuc Roof",color:"#7a8078",height:12,category:"structure",nuc:!0},{id:"nuc-floor",name:"Nuc Floor",color:"#8B8580",height:8,category:"structure",nuc:!0},{id:"nuc-stand",name:"Nuc Stand",color:"#A09080",height:20,category:"structure",nuc:!0},{id:"nuc-brood",name:"Nuc Brood Box",color:"#90968b",height:40,category:"box",nuc:!0},{id:"nuc-super",name:"Nuc Super",color:"#90968b",height:26,category:"box",nuc:!0},{id:"nuc-eke",name:"Nuc Eke",color:"#A89068",height:14,category:"accessory",nuc:!0}],Xe=["Swarm","Local","Home Bred","Buckfast","Native Black Bee","Carniolan","Italian","F1 Buckfast","Premium F1 Buckfast","UK F1 Buckfast","Premium UK F1 Buckfast","VSH Buckfast","UK Mated VSH Buckfast","Obsidian","Unknown"],et=["National","14x12","Commercial","Langstroth","WBC","Top Bar","Poly","Other"],tt=["Bred","Purchased","Swarm","Supersedure","Emergency Cell","Gift","Split","Unknown"],st=["Blue","White","Yellow","Red","Green","Pink"];function O(){return B}function H(e){return B.find(s=>s.id===e)}function Ee(e){return e.id=e.id||"hive-"+Date.now(),e.dateAdded=e.dateAdded||new Date().toISOString().slice(0,10),e.status=e.status||"Active",B.push(e),k("hives","create",W(e)),e}function K(e,s){const t=B.findIndex(n=>n.id===e);return t===-1?null:(B[t]={...B[t],...s},k("hives","update",W(B[t]),e),B[t])}function it(e){B=B.filter(s=>s.id!==e),k("hives","delete",null,e)}function nt(e,s,t){const n=B.find(o=>o.id===e);if(!n)return;n.components||(n.components=[]);const i=Math.max(0,n.components.length-1);n.components.splice(i,0,{type:s}),k("hives","update",W(n),e)}function ve(e,s){const t=B.find(n=>n.id===e);!t||!t.components||(t.components.splice(s,1),k("hives","update",W(t),e))}function pe(e,s,t){const n=B.find(o=>o.id===e);if(!n||!n.components||t<0||t>=n.components.length)return;const[i]=n.components.splice(s,1);n.components.splice(t,0,i),k("hives","update",W(n),e)}function j(e){const s={...e,id:"insp-"+Date.now(),date:e.date||new Date().toISOString().slice(0,10)};P.unshift(s),k("inspections","create",$e(s))}function at(e,s,t){const n=H(e);if(!n)return null;const i=Ee({hiveName:s,type:"Nuc",hiveStyle:n.hiveStyle,beeType:n.beeType,color:"#06b6d4",strength:50,queenMarked:!1,queenColor:null,queenYear:new Date().getFullYear(),orientation:"vertical",components:[{type:"nuc-roof"},{type:"nuc-brood"},{type:"nuc-floor"},{type:"nuc-stand"}]});return j({type:"Split",hive:n.hiveName,notes:`Split made — new nuc "${s}" created. ${t||""}`.trim(),strength:n.strength,queenSeen:!1,broodSpotted:!1}),j({type:"Split",hive:s,notes:`Split from ${n.hiveName}. ${t||""}`.trim(),strength:50,queenSeen:!1,broodSpotted:!1}),i}function ot(e,s,t){const n=H(e),i=H(s);!n||!i||(j({type:"Combined",hive:n.hiveName,notes:`Combined with ${i.hiveName}. ${t||""}`.trim(),strength:n.strength,queenSeen:!1,broodSpotted:!1}),j({type:"Combined",hive:i.hiveName,notes:`Combined into ${n.hiveName}. Hive deactivated. ${t||""}`.trim(),strength:0,queenSeen:!1,broodSpotted:!1}),K(s,{status:"Combined"}))}function lt(e,s){const t=H(e);t&&(j({type:"Hive Death",hive:t.hiveName,notes:s||"Colony died out.",strength:0,queenSeen:!1,broodSpotted:!1}),K(e,{status:"Dead",strength:0}))}function rt(e,s,t){const n=H(e);n&&j({type:"Moved",hive:n.hiveName,notes:`Moved to ${s}. ${t||""}`.trim(),strength:n.strength,queenSeen:!1,broodSpotted:!1})}const dt={"hive-roof":"nuc-roof","hive-floor":"nuc-floor","hive-stand":"nuc-stand",super:"nuc-super","national-brood":"nuc-brood","14x12-brood":"nuc-brood","hive-eke":"nuc-eke"},ct={"nuc-roof":"hive-roof","nuc-floor":"hive-floor","nuc-stand":"hive-stand","nuc-brood":"national-brood","nuc-super":"super","nuc-eke":"hive-eke"};function ut(e,s){const t=H(e);if(!t)return null;const n=t.type==="Hive",i=n?dt:ct,o=n?"Nuc":"Hive",a=(t.components||[]).map(l=>{if(n&&l.type==="queen-excluder")return null;const d=i[l.type];return d?{type:d}:l}).filter(Boolean);return j({type:"Converted",hive:t.hiveName,notes:`${n?"Downsized to Nuc":"Upgraded to Hive"}. ${s||""}`.trim(),strength:t.strength,queenSeen:!1,broodSpotted:!1}),K(e,{type:o,components:a}),H(e)}function Be(e=!1){return _.filter(s=>e||!s.deleted)}function vt(e){const s=_.find(t=>t.id===e);s&&(s.pinned=!s.pinned,k("notes","update",R(s),s._dvId||e))}function pt(e){const s=_.find(t=>t.id===e);s&&(s.deleted=!0,k("notes","update",R(s),s._dvId||e))}function mt(e,s){const t=_.find(n=>n.id===e);t&&(t.text=s,t.date=new Date().toISOString().slice(0,10),k("notes","update",R(t),t._dvId||e))}function ht(e,s=null){const t={id:"n"+Date.now(),text:e,date:new Date().toISOString().slice(0,10),pinned:!1,deleted:!1,hiveId:s};_.unshift(t),k("notes","create",R(t))}function gt(e){return _.find(s=>s.hiveId===e&&!s.deleted)||null}function me(e,s){const t=_.find(n=>n.hiveId===e&&!n.deleted);if(t)s?(t.text=s,t.date=new Date().toISOString().slice(0,10),t.pinned=!0):t.deleted=!0,k("notes","update",R(t),t._dvId||t.id);else if(s){const n={id:"n"+Date.now(),text:s,date:new Date().toISOString().slice(0,10),pinned:!0,deleted:!1,hiveId:e};_.unshift(n),k("notes","create",R(n))}}function Ae(e=!1){const s=F.filter(t=>!t.deleted);return e?s:s.filter(t=>!t.done)}function Le(e){const s=F.find(t=>t.id===e);s&&(s.done=!s.done,k("tasks","update",oe(s),s._dvId||e))}function xt(e){const s=F.find(t=>t.id===e);s&&(s.deleted=!0,k("tasks","update",oe(s),s._dvId||e))}function ft(e,s){const t={id:"t"+Date.now(),text:e,done:!1,due:s,deleted:!1};F.unshift(t),k("tasks","create",oe(t))}function bt(){return[{id:"esp32-1",name:"ESP32 Hive Scale",type:"ESP32",location:"Hive 5 - Survivor",status:"Online",battery:4.1,lastSeen:"2026-03-18T09:30:00Z",firmware:"v1.0.0",ip:"192.168.1.45"},{id:"sb-inside",name:"SwitchBot Inside",type:"SwitchBot",location:"Hive 5 - Survivor (inside)",status:"Online",battery:87,lastSeen:"2026-03-18T09:25:00Z",temp:32.5,humidity:68},{id:"sb-outside",name:"SwitchBot Outside",type:"SwitchBot",location:"Apiary (ambient)",status:"Online",battery:92,lastSeen:"2026-03-18T09:28:00Z",temp:14.2,humidity:55}]}const yt=["0e98bf2afda73d29f786212cd02fc542cb3307fea44f76f208f396b5fcd4ea98"];async function wt(e,s){const n=new TextEncoder().encode(`${e}:${s}`),i=await crypto.subtle.digest("SHA-256",n);return Array.from(new Uint8Array(i)).map(o=>o.toString(16).padStart(2,"0")).join("")}async function kt(e){e.innerHTML=`
  ...
  `,document.getElementById("loginForm").addEventListener("submit",async t=>{t.preventDefault();const n=document.getElementById("username").value.trim().toLowerCase(),i=document.getElementById("password").value,o=await wt(n,i);if(yt.includes(o))sessionStorage.setItem("hive_user",JSON.stringify({name:n.charAt(0).toUpperCase()+n.slice(1),role:"admin"})),window.location.hash="#/apiary";else{const a=document.getElementById("loginError");a.textContent="Invalid username or password",a.classList.remove("hidden")}}),document.getElementById("username").focus()}function $t(e){const s=e>=80?"Strong":e>=50?"Fair":"Weak";return`<span class="inline-flex items-center gap-1.5">
  ...
  </span>`}function C(e,s=!1,t=!1,n="#/apiary"){return`
    ...
    <div id="acc-${e}" class="accordion-content ${n?"open":""}">${t}</div>
      ...
      <div class="text-gray-600 text-xs text-center py-4">No components</div>
      ...
      </div>`;else if(f)c+=`<div style="width:${l+Math.round(4*o)}px; height:${m}px; background:${h.color}; border-radius: 0 0 2px 2px"></div>`;else if(u){const w=Math.max(2,Math.round(3*o));c+=`<div class="flex justify-between" style="width:${l+Math.round(12*o)}px; height:${m}px">
        <div style="width:${w}px; background:${h.color}; height:${m}px; border-radius: 0 0 2px 2px"></div>
        <div class="flex-1 mx-0.5" style="background:${h.color}; height:${Math.round(m*.3)}px; border-radius: 0 0 2px 2px"></div>
        <div style="width:${w}px; background:${h.color}; height:${m}px; border-radius: 0 0 2px 2px"></div>
      </div>`}else if(y){const w=Math.max(7,Math.round(8*o));c+=`<div class="relative flex items-center justify-center" style="width:${l}px; height:${m}px; background:${h.color}">
        <span class="text-white/60 font-medium text-center leading-tight" style="font-size:${w}px">${h.name}</span>
        ${n?he(i,v):""}
      </div>`}else{const w=Math.max(8,Math.round(10*o));c+=`<div class="relative flex items-center justify-center" style="width:${l}px; height:${m}px; background:${h.color}; border-left:2px solid rgba(0,0,0,0.12); border-right:2px solid rgba(0,0,0,0.12)">
        <span class="text-white font-semibold text-center leading-tight" style="font-size:${w}px; text-shadow:0 1px 2px rgba(0,0,0,0.3)">${h.name}</span>
        ${n?he(i,v):""}
      </div>`}}),c+="</div>",c}function he(e,s){return`<button onclick="document.dispatchEvent(new CustomEvent('hive-remove-component', {detail:{hiveId:'${e}',index:${s}}}))" class="absolute top-0 right-0.5 text-white/40 hover:text-white text-xs leading-none p-0.5" title="Remove">✕</button>`}function Me(e=[],s="#f59e0b"){if(!e||!e.length)return'<div class="w-full h-full flex items-center justify-center text-3xl">🏠</div>';const t=.35,n=Ne(e),i=Math.round((n?ie:se)*t);let o='<div class="flex flex-col items-center justify-end h-full py-1">';return e.forEach(a=>{const l=le(a.type);if(!l)return;const d=n?Ct(l):l.height,c=Math.round(d*t);l.id.includes("roof")?o+=`<div class="rounded-t" style="width:${i+3}px; height:${c}px; background:${l.color}"></div>`:l.id.includes("stand")?o+=`<div class="flex justify-between" style="width:${i+6}px; height:${c}px">
        ...
        <div style="width:1px; background:${l.color}; height:${c}px"></div>
      </div>`:l.id.includes("floor")?o+=`<div style="width:${i+2}px; height:${Math.max(2,c)}px; background:${l.color}"></div>`:l.category==="accessory"?o+=`<div style="width:${i}px; height:${Math.max(2,c)}px; background:${l.color}"></div>`:o+=`<div style="width:${i}px; height:${c}px; background:${l.color}; border-left:1px solid rgba(0,0,0,0.12); border-right:1px solid rgba(0,0,0,0.12)"></div>`}),o+="</div>",o}const Et={0:"Clear Sky",1:"Mainly Clear",2:"Partly Cloudy",3:"Overcast",45:"Foggy",48:"Rime Fog",51:"Light Drizzle",53:"Drizzle",55:"Heavy Drizzle",61:"Light Rain",63:"Rain",65:"Heavy Rain",71:"Light Snow",73:"Snow",75:"Heavy Snow",77:"Snow Grains",80:"Light Showers",81:"Showers",82:"Heavy Showers",85:"Light Snow Showers",86:"Snow Showers",95:"Thunderstorm",96:"Thunderstorm + Hail",99:"Thunderstorm + Heavy Hail"};let G=null,ge=0;const Bt=10*60*1e3;async function He(e,s){if(G&&Date.now()-ge<Bt)return G;const t=`https://api.open-meteo.com/v1/forecast?latitude=${e}&longitude=${s}&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m&timezone=auto`,n=await fetch(t);if(!n.ok)throw new Error(`Weather API: HTTP ${n.status}`);const o=(await n.json()).current,a=o.weather_code;return G={temp:Math.round(o.temperature_2m),conditions:Et[a]||`Code ${a}`,humidity:o.relative_humidity_2m,windSpeed:Math.round(o.wind_speed_10m),icon:At(a)},ge=Date.now(),G}function At(e){return e===0||e===1?"☀️":e===2?"⛅":e===3?"☁️":e>=45&&e<=48?"🌫️":e>=51&&e<=55?"🌦️":e>=61&&e<=65?"🌧️":e>=71&&e<=77?"🌨️":e>=80&&e<=82?"🌦️":e>=85&&e<=86?"🌨️":e>=95?"⛈️":"🌤️"}async function Lt(e){const s=O(),t=Je(),n=Be(),i=Ae(),o=s.filter(a=>a.status==="Active");e.innerHTML=`
    ${C(D.name,!1,!0)}
    ...
    <main class="max-w-6xl mx-auto pb-8 hex-bg">
      ...
      <section class="px-5 mb-8">
        <div class="flex items-center justify-between mb-4">
          <h2 class="section-title">${o.length} Hive${o.length!==1?"s":""}</h2>
          <a href="#/admin" class="section-subtitle text-hive-gold hover:opacity-80">Manage</a>
        </div>
        <div class="flex gap-3 overflow-x-auto no-scrollbar pb-2">
          ${o.map(a=>`
            <a href="#/hive/${encodeURIComponent(a.hiveName)}" class="flex-shrink-0 w-[170px] animate-in">
              <div class="card p-4 flex flex-col items-center" style="min-height:220px">
                <div class="w-full h-24 flex items-end justify-center mb-3">${Me(a.components,a.color)}</div>
                <h3 class="font-serif text-sm font-medium text-hive-text text-center leading-tight mb-1.5">${a.hiveName}</h3>
                <div class="mb-1">${$t(a.strength)}</div>
                <p class="section-subtitle mt-0.5 text-center leading-tight">${a.beeType}</p>
              </div>
            </a>
          `).join("")}
        </div>
      ...
      <div class="px-5 grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
        ...
        <section>
          ...
          <div class="card space-y-3">
            ${n.filter(a=>a.pinned&&!a.deleted).slice(0,3).map(a=>`
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1 min-w-0">
                  <p class="text-sm leading-relaxed text-hive-text">${a.text}</p>
                  <p class="text-[11px] text-hive-muted mt-1">${new Date(a.date).toLocaleDateString("en-GB",{month:"short",day:"numeric",year:"numeric"})}</p>
                </div>
              </div>
            `).join("")||`
        ...
        <section>
          ...
          <div class="card space-y-3" id="taskList">
            ${i.map(a=>`
              <label class="flex items-center gap-3 cursor-pointer task-item" data-task-id="${a.id}">
                <input type="checkbox" class="w-4 h-4 rounded accent-[var(--hive-gold)] task-check" data-task="${a.id}">
                <span class="text-sm flex-1 text-hive-text">${a.text}</span>
                <span class="text-[10px] uppercase tracking-wider text-hive-gold">${Q(a.due)}</span>
              </label>
            `).join("")||`
              <div class="text-center py-4">
      ...
      <section class="px-5 mb-6">
        ...
        <div class="space-y-2">
          ${t.slice(0,5).map(a=>`
            <a href="#/inspections?date=${a.date}" class="card flex items-center gap-3 p-4 block">
              ...
              <div class="flex-1">
                <div class="text-sm text-hive-text">${a.count} inspection${a.count>1?"s":""}</div>
                <div class="text-[11px] text-hive-muted">${new Date(a.date).toLocaleDateString("en-GB",{month:"short",day:"numeric",year:"numeric"})}</div>
              </div>
              ${q.chevron}
            </a>
          `).join("")}
  ...
  `,He(D.lat,D.lng).then(a=>{const l=document.getElementById("liveWeather");l&&(l.textContent=`${a.temp}°C ${a.conditions}`)}).catch(()=>{const a=document.getElementById("liveWeather");a&&(a.textContent="—")}),document.querySelectorAll(".task-check").forEach(a=>{a.addEventListener("change",()=>{const l=a.dataset.task;Le(l);const d=a.closest(".task-item");d&&(d.style.opacity="0.3",d.style.textDecoration="line-through",setTimeout(()=>d.remove(),600))})})}async function ne(e,s){var u,f,y,w,$,S,I,N,M,g,E;const t=s.id,i=O().find(x=>x.hiveName===t),o=[...Ke(t),...Ce().filter(x=>x.hive===t)].sort((x,b)=>new Date(b.date)-new Date(x.date));if(!i){e.innerHTML=`
      ${C("Not Found",!0)}
      ...
      </div>`;return}const a=i.strength>=80?"#22c55e":i.strength>=50?"#f59e0b":"#ef4444",l=i.queenYear?new Date().getFullYear()-i.queenYear:null,d=o.filter(x=>x.type==="Inspection"),c=d.length?new Date(d[0].date):null,p=c?Math.floor((new Date-c)/864e5):null,v=gt(i.id);e.innerHTML=`
    ${C(i.hiveName,!0)}
    ...
    <main class="max-w-6xl mx-auto pb-24">
      ...
      <section class="p-4">
        <div class="rounded-2xl overflow-hidden" style="background: var(--hive-surface)">
          ...
          <div class="h-2 w-full" style="background: ${a}"></div>
        </div>
        ...
        <div class="mt-4 flex items-center justify-between">
          ...
          <div class="text-right">
            <div class="text-3xl font-bold" style="color: ${a}">${i.strength}%</div>
      ...
      <section class="px-4 mb-6">
        ...
        <div class="space-y-0">
          ${o.map((x,b)=>`
            <a href="#/inspection/${x.id||b}?from=${encodeURIComponent(t)}" class="flex gap-3 ${b<o.length-1?"pb-4":""} block">
              <div class="flex flex-col items-center">
                ...
                ${b<o.length-1?'<div class="w-px bg-hive-border flex-1 mt-1"></div>':""}
              </div>
          ...
          ${o.length===0?'<p class="text-hive-muted text-sm text-center py-6">No activity recorded yet.</p>':""}
        </div>
      </section>
  ...
  `;const h=document.getElementById("noteModal"),m=()=>h==null?void 0:h.classList.remove("hidden"),r=()=>h==null?void 0:h.classList.add("hidden");(u=document.getElementById("editHiveNote"))==null||u.addEventListener("click",m),(f=document.getElementById("noteModalBackdrop"))==null||f.addEventListener("click",r),(y=document.getElementById("noteModalClose"))==null||y.addEventListener("click",r),(w=document.getElementById("noteModalCancel"))==null||w.addEventListener("click",r),($=document.getElementById("noteModalSave"))==null||$.addEventListener("click",()=>{var b;const x=(b=document.getElementById("noteModalText"))==null?void 0:b.value.trim();x&&(me(i.id,x),r(),ne(e,s))}),(S=document.getElementById("noteModalDelete"))==null||S.addEventListener("click",()=>{confirm("Delete this hive note?")&&(me(i.id,""),r(),ne(e,s))}),(I=e.querySelector('[data-op="split"]'))==null||I.addEventListener("click",()=>{const x=prompt('Name for the new nuc/hive (e.g. "Nuc 2 - Split"):');if(!x)return;const b=prompt("Split notes (optional):")||"";at(i.id,x,b),window.location.hash="#/apiary"}),(N=e.querySelector('[data-op="combine"]'))==null||N.addEventListener("click",()=>{const x=O().filter(z=>z.id!==i.id&&z.status==="Active");if(!x.length){alert("No other active hives to combine with.");return}const b=x.map((z,_e)=>`${_e+1}. ${z.hiveName}`).join(`
`),L=prompt(`Combine ${i.hiveName} INTO which hive? (this hive will be deactivated)

${b}

Enter number:`);if(!L)return;const T=parseInt(L,10)-1;if(T<0||T>=x.length){alert("Invalid selection.");return}const Te=prompt("Combine notes (optional):")||"";ot(x[T].id,i.id,Te),window.location.hash="#/apiary"}),(M=e.querySelector('[data-op="dead"]'))==null||M.addEventListener("click",()=>{if(!confirm(`Mark "${i.hiveName}" as dead? This will deactivate the hive.`))return;const x=prompt("Notes on the death (optional):")||"";lt(i.id,x),window.location.hash="#/apiary"}),(g=e.querySelector('[data-op="move"]'))==null||g.addEventListener("click",()=>{const x=prompt("Where is the hive being moved to?");if(!x)return;const b=prompt("Move notes (optional):")||"";rt(i.id,x,b),alert(`${i.hiveName} move recorded to timeline.`),window.location.hash="#/hive/"+encodeURIComponent(i.hiveName)}),(E=e.querySelector('[data-op="convert"]'))==null||E.addEventListener("click",()=>{const x=i.type==="Hive"?"downsize to a Nuc":"upgrade to a full Hive";if(!confirm(`Convert "${i.hiveName}" — ${x}?

All components will be swapped to the matching type.`))return;const b=prompt("Conversion notes (optional):")||"",L=ut(i.id,b);L&&(window.location.hash="#/hive/"+encodeURIComponent(L.hiveName))})}const It="modulepreload",Nt=function(e){return"/"+e},xe={},fe=function(s,t,n){let i=Promise.resolve();if(t&&t.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),l=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));i=Promise.allSettled(t.map(d=>{if(d=Nt(d),d in xe)return;xe[d]=!0;const c=d.endsWith(".css"),p=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${p}`))return;const v=document.createElement("link");if(v.rel=c?"stylesheet":It,c||(v.as="script"),v.crossOrigin="",v.href=d,l&&v.setAttribute("nonce",l),document.head.appendChild(v),c)return new Promise((h,m)=>{v.addEventListener("load",h),v.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=a,window.dispatchEvent(l),!l.defaultPrevented)throw a}return i.then(a=>{for(const l of a||[])l.status==="rejected"&&o(l.reason);return s().catch(o)})};async function Mt(e){e.innerHTML=`
    ${C("Sensor Dashboard",!0)}
  ...
  `;const{Chart:s,registerables:t}=await fe(async()=>{const{Chart:p,registerables:v}=await import("./chart-19k6OvwP.js");return{Chart:p,registerables:v}},[]);await fe(()=>import("./chartjs-adapter-date-fns.esm-CV7ru7NP.js"),__vite__mapDeps([0,1])),s.register(...t);const n={responsive:!0,maintainAspectRatio:!0,aspectRatio:2.5,interaction:{mode:"index",intersect:!1},plugins:{legend:{labels:{color:"#9ca3af",usePointStyle:!0,padding:12}},tooltip:{backgroundColor:"#1a1d27",borderColor:"#2a2e3e",borderWidth:1,titleColor:"#e4e4e7",bodyColor:"#e4e4e7",padding:10,callbacks:{title:p=>new Date(p[0].parsed.x).toLocaleString()}}},scales:{x:{type:"time",time:{tooltipFormat:"dd MMM HH:mm"},grid:{color:"rgba(255,255,255,0.05)"},ticks:{color:"#9ca3af",maxTicksLimit:8}},y:{grid:{color:"rgba(255,255,255,0.05)"},ticks:{color:"#9ca3af"}}}},i=(p,v)=>({label:p,borderColor:v,backgroundColor:v+"1a",fill:!1,tension:.3,pointRadius:0,borderWidth:2,data:[]}),o=new s(document.getElementById("weightChart"),{type:"line",data:{datasets:[{...i("Weight (kg)","#f59e0b"),fill:!0}]},options:{...n,scales:{...n.scales,y:{...n.scales.y,title:{display:!0,text:"kg",color:"#9ca3af"}}}}}),a=new s(document.getElementById("tempChart"),{type:"line",data:{datasets:[i("Internal (°C)","#ef4444"),i("Leg (°C)","#a78bfa")]},options:{...n,aspectRatio:2,scales:{...n.scales,y:{...n.scales.y,title:{display:!0,text:"°C",color:"#9ca3af"}}}}}),l=new s(document.getElementById("envChart"),{type:"line",data:{datasets:[{...i("Humidity (%)","#3b82f6"),yAxisID:"yHum"},{...i("Battery (V)","#22c55e"),yAxisID:"yBat"}]},options:{...n,aspectRatio:2,scales:{x:n.scales.x,yHum:{type:"linear",position:"left",grid:{color:"rgba(255,255,255,0.05)"},ticks:{color:"#3b82f6"},title:{display:!0,text:"%",color:"#3b82f6"}},yBat:{type:"linear",position:"right",grid:{drawOnChartArea:!1},ticks:{color:"#22c55e"},title:{display:!0,text:"V",color:"#22c55e"}}}}});async function d(){const p=parseInt(document.getElementById("timeRange").value,10),v=new Date(Date.now()-p*3600*1e3);try{const h=await Qe();document.getElementById("errorBanner").classList.add("hidden");const m=h.filter(u=>new Date(u.timestamp)>=v);if(m.length>0){const u=m[m.length-1],f=(w,$,S)=>{document.getElementById(w).textContent=$!=null?Number($).toFixed(S):"—"};f("latestWeight",u.weight,1),f("latestTemp",u.internalTemp,1),f("latestHum",u.hiveHum,1),f("latestBat",u.batteryVoltage,2),f("latestLeg",u.legTemp,1);const y=Math.floor((Date.now()-new Date(u.timestamp))/1e3);document.getElementById("lastReading").textContent=y<60?`${y}s ago`:y<3600?`${Math.floor(y/60)}m ago`:`${Math.floor(y/3600)}h ago`,document.getElementById("dataPoints").textContent=`${m.length} pts`}const r=(u,f)=>({x:new Date(u.timestamp),y:u[f]});o.data.datasets[0].data=m.map(u=>r(u,"weight")),a.data.datasets[0].data=m.map(u=>r(u,"internalTemp")),a.data.datasets[1].data=m.map(u=>r(u,"legTemp")),l.data.datasets[0].data=m.map(u=>r(u,"hiveHum")),l.data.datasets[1].data=m.map(u=>r(u,"batteryVoltage")),o.update("none"),a.update("none"),l.update("none")}catch(h){const m=document.getElementById("errorBanner");m.textContent=`Failed to load sensor data: ${h.message}`,m.classList.remove("hidden")}}d();const c=setInterval(d,5*60*1e3);return document.getElementById("timeRange").addEventListener("change",d),document.getElementById("refreshBtn").addEventListener("click",d),()=>{clearInterval(c),o.destroy(),a.destroy(),l.destroy()}}function X(e,s=3e3){const t=document.getElementById("toast");t&&t.remove();const n=document.createElement("div");n.id="toast",n.className="fixed top-20 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 rounded-full text-sm font-medium animate-in",n.style.cssText="background:var(--hive-gold);color:#0B0D0E;box-shadow:0 4px 20px rgba(212,175,55,0.3)",n.textContent=e,document.body.appendChild(n),setTimeout(()=>{n.style.opacity="0",setTimeout(()=>n.remove(),300)},s)}const Ht=["Varroa mites","Nosema","Chalkbrood","Stonebrood","American Foulbrood","European Foulbrood"],Tt=["Waxmoth","Mice","Ants","Wasps","Small Hive Beetle","Hornets"],_t=["","Compact","Spotty","Patchy","Drone-heavy","None visible"];async function Dt(e){const t=new URLSearchParams(window.location.hash.split("?")[1]||"").get("hive")||"",i=O().filter(r=>r.status==="Active").map(r=>r.hiveName);e.innerHTML=`
    ${C("New Inspection",!0)}
  ...
  `,Ie(e);const o={queenSeen:!1,broodSpotted:!1,queenCells:!1};document.querySelectorAll("[data-health]").forEach(r=>{r.addEventListener("change",()=>{o[r.dataset.health]=r.checked})});let a="";document.querySelectorAll(".temperament-pill").forEach(r=>{r.addEventListener("click",()=>{a=r.dataset.temperament,document.querySelectorAll(".temperament-pill").forEach(u=>u.className="temperament-pill btn-secondary flex-1 py-2 text-xs"),r.className="temperament-pill btn-primary flex-1 py-2 text-xs"})});const l=document.getElementById("strengthSlider"),d=document.getElementById("strengthValue");l.addEventListener("input",()=>{d.textContent=l.value+"%"});const c=document.getElementById("weightLeft"),p=document.getElementById("weightRight"),v=document.getElementById("weightTotal"),h=()=>{const r=parseFloat(c.value)||0,u=parseFloat(p.value)||0;(r>0||u>0)&&(v.value=(r+u).toFixed(2))};c.addEventListener("input",h),p.addEventListener("input",h),document.getElementById("fetchWeightBtn").addEventListener("click",()=>X("IoT weight fetch not yet connected"));async function m(){const r=document.getElementById("weatherStatus");try{r.textContent="Fetching...";const u=await He(D.lat,D.lng);document.getElementById("weatherConditions").value=u.conditions,document.getElementById("weatherTemp").value=u.temp,r.innerHTML='<span style="color:var(--hive-sage)">&#9679;</span> Auto-filled: '+u.temp+"°C "+u.conditions+" ("+u.humidity+"% humidity, "+u.windSpeed+" km/h wind)"}catch{r.textContent="Could not fetch weather."}}m(),document.getElementById("refreshWeatherBtn").addEventListener("click",m),document.getElementById("inspectionForm").addEventListener("submit",r=>{r.preventDefault();const u=document.getElementById("hiveSelect").value;if(!u){X("Please select a hive");return}const f=[];document.querySelectorAll("[data-disease]:checked").forEach($=>f.push($.dataset.disease));const y=[];document.querySelectorAll("[data-pest]:checked").forEach($=>y.push($.dataset.pest));const w={date:new Date().toISOString().slice(0,10),type:"Inspection",hive:u,strength:parseInt(l.value,10),queenSeen:o.queenSeen,broodSpotted:o.broodSpotted,queenCells:o.queenCells,temperament:a,broodPattern:document.getElementById("broodPattern").value,weightLeft:parseFloat(c.value)||null,weightRight:parseFloat(p.value)||null,weightTotal:parseFloat(v.value)||null,diseases:f,pests:y,notes:document.getElementById("notes").value,weatherTemp:parseFloat(document.getElementById("weatherTemp").value)||null,weatherConditions:document.getElementById("weatherConditions").value||null};Ze(w),X("Inspection saved for "+u),setTimeout(()=>{window.location.hash="#/apiary"},1500)})}function qt(e){const s=O(),t=s.filter(i=>i.type==="Hive"&&i.status==="Active"),n=s.filter(i=>i.type==="Nuc"&&i.status==="Active");e.innerHTML=`
    ${C("Admin",!0)}
  ...
  `}function Rt(e,s){const t=s.id;let n=H(t);if(!n){e.innerHTML=`${C("Not Found",!0)}<div class="max-w-6xl mx-auto p-4 text-center py-16"><p class="text-hive-muted">Hive not found</p><a href="#/apiary" class="btn-primary inline-block mt-4">Back</a></div>`;return}function i(){n=H(t),e.innerHTML=`
      ${C("Build "+n.hiveName,!0,!1,"#/hive/"+encodeURIComponent(n.hiveName))}
      <main class="max-w-6xl mx-auto p-4 pb-8">
        ...
        <section class="card-surface mb-6">
          ...
          <div class="space-y-2">
            ${te.filter(a=>{const l=n.type==="Nuc";return!(a.nuc!==l||a.id.includes("roof")||a.id.includes("stand")||a.id.includes("floor"))}).map(a=>`
              <button data-add-type="${a.id}" class="w-full flex items-center justify-between p-3 rounded-lg hover:bg-hive-surface-hover transition-colors border border-hive-border">
                <div class="flex items-center gap-3">
                  <div class="w-12 h-6 rounded" style="background:${a.color}"></div>
                  <span class="text-sm font-medium text-hive-text">${a.name}</span>
                </div>
                <span class="text-xs text-hive-muted">${a.category}</span>
              </button>
            `).join("")}
          </div>
        </section>
        ...
        <section class="card-surface">
          ...
          <div class="space-y-1">
            ${(n.components||[]).map((a,l)=>{const d=te.find(p=>p.id===a.type);if(!d)return"";const c=(n.components||[]).length;return`
    ...
    `,e.querySelectorAll("[data-add-type]").forEach(a=>{a.addEventListener("click",()=>{nt(t,a.dataset.addType),i()})}),e.querySelectorAll("[data-remove-idx]").forEach(a=>{a.addEventListener("click",()=>{ve(t,parseInt(a.dataset.removeIdx,10)),i()})}),e.querySelectorAll("[data-move-up]").forEach(a=>{a.addEventListener("click",()=>{const l=parseInt(a.dataset.moveUp,10);pe(t,l,l-1),i()})}),e.querySelectorAll("[data-move-down]").forEach(a=>{a.addEventListener("click",()=>{const l=parseInt(a.dataset.moveDown,10);pe(t,l,l+1),i()})})}i();const o=a=>{a.detail.hiveId===t&&(ve(t,a.detail.index),i())};return document.addEventListener("hive-remove-component",o),()=>document.removeEventListener("hive-remove-component",o)}function Pt(e,s){var l;const t=s.id,n=t&&t!=="new",o=(n?H(t):null)||{hiveName:"",type:"Hive",hiveStyle:"National",beeType:"Buckfast",strength:100,color:"#f59e0b",queenMarked:!1,queenColor:null,queenYear:new Date().getFullYear(),components:[{type:"roof"},{type:"deep-box"},{type:"stand"}]};e.innerHTML=`
    ${C(n?"Edit "+o.hiveName:"Add New Hive",!0,!1,n?"#/hive/"+encodeURIComponent(o.hiveName):"#/admin")}
    ...
    <main class="max-w-6xl mx-auto p-4 pb-8">
      <form id="hiveForm" class="space-y-6">
        ...
        <section class="flex justify-center py-6 rounded-2xl" style="background: var(--hive-surface)">
          ${re(o.components,{size:"md"})}
        </section>

        <!-- Name -->
        <section class="card-surface">
          <label class="block section-subtitle mb-2">Hive Name</label>
          <input type="text" id="hiveName" class="input-field text-lg" value="${o.hiveName}" placeholder="e.g. Hive 7 - Luna" required>
        </section>
        ...
        <section class="card-surface grid grid-cols-2 gap-4">
          <div>
            ...
            <select id="hiveType" class="input-field">
              <option value="Hive" ${o.type==="Hive"?"selected":""}>Hive</option>
              <option value="Nuc" ${o.type==="Nuc"?"selected":""}>Nuc</option>
            </select>
          </div>
          <div>
            <label class="block section-subtitle mb-2">Hive Style</label>
            <select id="hiveStyle" class="input-field">
              ${et.map(d=>`<option value="${d}" ${o.hiveStyle===d?"selected":""}>${d}</option>`).join("")}
            </select>
          </div>
        </section>
        ...
        <section class="card-surface">
          ...
          <div class="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <select id="beeType" class="input-field">
                ${Xe.map(d=>`<option value="${d}" ${o.beeType===d?"selected":""}>${d}</option>`).join("")}
              </select>
            </div>
            <div>
              <label class="block text-xs text-hive-muted mb-1">Source</label>
              <select id="queenSource" class="input-field">
                ${tt.map(d=>`<option value="${d}" ${o.queenSource===d?"selected":""}>${d}</option>`).join("")}
              </select>
            </div>
          </div>
          <div class="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <input type="number" id="queenYear" class="input-field" value="${o.queenYear||""}" min="2018" max="2030">
            </div>
            <div>
              <label class="block text-xs text-hive-muted mb-1">Date Added</label>
              <input type="date" id="queenAddedDate" class="input-field" value="${o.queenAddedDate||""}">
            </div>
          </div>
          <div class="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <select id="queenMarked" class="input-field">
                <option value="false" ${o.queenMarked?"":"selected"}>No</option>
                <option value="true" ${o.queenMarked?"selected":""}>Yes</option>
              </select>
            </div>
            <div>
              ...
              <select id="queenColor" class="input-field">
                ...
                ${st.map(d=>`<option value="${d}" ${o.queenColor===d?"selected":""}>${d}</option>`).join("")}
              </select>
            </div>
          </div>
          <div class="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <select id="queenClipped" class="input-field">
                <option value="false" ${o.queenClipped?"":"selected"}>No</option>
                <option value="true" ${o.queenClipped?"selected":""}>Yes</option>
              </select>
            </div>
            <div></div>
          </div>
          <div>
            ...
            <textarea id="queenNotes" class="input-field" rows="2" placeholder="e.g. Gentle temperament, prolific layer">${o.queenNotes||""}</textarea>
          </div>
        </section>
        ...
        <section class="card-surface">
          ...
          <div class="flex items-center gap-4">
            <input type="range" id="strengthSlider" min="0" max="100" value="${o.strength}" class="flex-1 accent-hive-amber">
            <span id="strengthVal" class="text-xl font-bold text-hive-amber w-14 text-right">${o.strength}%</span>
          </div>
        </section>
        ...
        <a href="#/build/${t}" class="card-surface flex items-center justify-between p-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="text-xs text-hive-muted">${(o.components||[]).length} components</div>
            </div>
          </div>
          <svg class="w-4 h-4 text-hive-muted" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
        </a>`:""}
        ...
        <section class="space-y-3">
          ...
          <a href="${n?"#/hive/"+encodeURIComponent(o.hiveName):"#/admin"}" class="btn-secondary w-full py-3 text-center block">Cancel</a>
  ...
  `;const a=document.getElementById("strengthSlider");a.addEventListener("input",()=>{document.getElementById("strengthVal").textContent=a.value+"%"}),document.getElementById("hiveForm").addEventListener("submit",d=>{d.preventDefault();const c={hiveName:document.getElementById("hiveName").value.trim(),type:document.getElementById("hiveType").value,hiveStyle:document.getElementById("hiveStyle").value,beeType:document.getElementById("beeType").value,color:o.color||"#f59e0b",strength:parseInt(a.value,10),queenYear:parseInt(document.getElementById("queenYear").value,10)||null,queenMarked:document.getElementById("queenMarked").value==="true",queenColor:document.getElementById("queenColor").value||null,queenClipped:document.getElementById("queenClipped").value==="true",queenSource:document.getElementById("queenSource").value||"Unknown",queenAddedDate:document.getElementById("queenAddedDate").value||null,queenNotes:document.getElementById("queenNotes").value.trim(),orientation:"vertical"};if(n)K(t,c),window.location.hash="#/hive/"+encodeURIComponent(c.hiveName);else{c.components=[{type:"roof"},{type:"deep-box"},{type:"stand"}];const p=Ee(c);window.location.hash="#/build/"+p.id}}),n&&((l=document.getElementById("deleteBtn"))==null||l.addEventListener("click",()=>{confirm(`Delete "${o.hiveName}"? This cannot be undone.`)&&(it(t),window.location.hash="#/apiary")}))}function Ot(e){function s(){var n;const t=Ae(!0);e.innerHTML=`
      ${C("Tasks",!0)}
    ...
    `,e.querySelectorAll(".toggle-task").forEach(i=>{i.addEventListener("change",()=>{Le(i.dataset.tid),s()})}),e.querySelectorAll(".delete-task").forEach(i=>{i.addEventListener("click",()=>{xt(i.dataset.tid),s()})}),(n=document.getElementById("addTaskBtn"))==null||n.addEventListener("click",()=>{const i=prompt("New task:");if(!i)return;const o=prompt("Due date (YYYY-MM-DD, optional):")||"";ft(i,o||null),s()})}s()}function Ft(e){function s(){var c,p,v,h,m;const t=Be(!1);e.innerHTML=`
      ${C("Notes",!0)}
      <main class="max-w-6xl mx-auto p-5 space-y-4">
        <div class="flex items-center justify-between mb-2">
    ...
    `,e.querySelectorAll(".pin-note").forEach(r=>{r.addEventListener("click",()=>{vt(r.dataset.nid),s()})}),e.querySelectorAll(".delete-note").forEach(r=>{r.addEventListener("click",()=>{pt(r.dataset.nid),s()})});const n=document.getElementById("noteModal"),i=document.getElementById("noteModalTitle"),o=document.getElementById("noteModalText");let a=null;const l=(r,u,f)=>{i.textContent=r,o.value=u,a=f,n==null||n.classList.remove("hidden"),setTimeout(()=>o.focus(),100)},d=()=>{n==null||n.classList.add("hidden"),a=null};(c=document.getElementById("addNoteBtn"))==null||c.addEventListener("click",()=>l("New Note","",null)),e.querySelectorAll(".edit-note").forEach(r=>{r.addEventListener("click",()=>l("Edit Note",r.dataset.ntext,r.dataset.nid))}),(p=document.getElementById("noteModalBackdrop"))==null||p.addEventListener("click",d),(v=document.getElementById("noteModalClose"))==null||v.addEventListener("click",d),(h=document.getElementById("noteModalCancel"))==null||h.addEventListener("click",d),(m=document.getElementById("noteModalSave"))==null||m.addEventListener("click",()=>{const r=o==null?void 0:o.value.trim();r&&(a?mt(a,r):ht(r),s())})}s()}function Wt(e){const t=new URLSearchParams(window.location.hash.split("?")[1]||"").get("date")||"";let n=Se().filter(o=>o.type==="Inspection");t&&(n=n.filter(o=>o.date===t));const i=t?`Inspections — ${new Date(t).toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"})}`:"All Inspections";e.innerHTML=`
    ${C(i,!0)}
    <main class="max-w-6xl mx-auto p-4 pb-8">
      <div class="flex items-center justify-between mb-4">
      ...
      <div class="space-y-2">
        ${n.map((o,a)=>`
          <a href="#/inspection/${o.id||a}${t?"?from=inspections&date="+t:""}" class="card-surface flex items-center justify-between p-3 block">
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <span class="text-sm font-medium text-hive-text">${o.hive}</span>
                <span class="pill-green text-[9px]">${o.strength||"—"}%</span>
              </div>
              <p class="text-xs text-hive-muted truncate">${o.notes||"No notes"}</p>
            </div>
            <div class="flex items-center gap-2 flex-shrink-0 ml-3">
              <span class="text-xs text-hive-muted">${new Date(o.date).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"})}</span>
              <svg class="w-4 h-4 text-hive-muted" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
            </div>
          </a>`).join("")}
  ...
  `}const zt=["Varroa mites","Nosema","Chalkbrood","Stonebrood","American Foulbrood","European Foulbrood"],Ut=["Waxmoth","Mice","Ants","Wasps","Small Hive Beetle","Hornets"],Vt=["","Compact","Spotty","Patchy","Drone-heavy","None visible"];function Yt(e,s){var M;const t=s.id,n=new URLSearchParams(window.location.hash.split("?")[1]||""),i=n.get("from")||"",o=n.get("date")||"",a=[...Se(),...Ce()],l=a.find(g=>g.id===t)||a[parseInt(t,10)];if(!l){e.innerHTML=`
      ${C("Not Found",!0)}
      <div class="max-w-6xl mx-auto p-4 text-center py-16">
        <div class="text-5xl mb-4">🔍</div>
      ...
      </div>`;return}let d="#/inspections";i==="inspections"&&o?d=`#/inspections?date=${o}`:i&&i!=="inspections"&&(d=`#/hive/${encodeURIComponent(i)}`);const c=(l.strength||0)>=80?"var(--hive-sage)":(l.strength||0)>=50?"var(--hive-gold)":"var(--hive-red)";e.innerHTML=`
    ${C(l.hive,!0,!1,d)}
    <main class="max-w-3xl mx-auto p-5 pb-8 space-y-5">
      ...
      <section class="card p-5 space-y-5">
        ...
        <div class="border-t pt-4" style="border-color:var(--hive-border)">
          ...
          <select id="broodPattern" class="input-field" disabled>
            ${Vt.map(g=>`<option value="${g}" ${l.broodPattern===g?"selected":""}>${g||"Not assessed"}</option>`).join("")}
          </select>
        </div>
      </section>
      ...
      <section class="card p-5">
        <h3 class="section-subtitle mb-2">Issues</h3>
        ${J("diseases","Diseases",`<div class="grid grid-cols-2 gap-2 pb-2">${zt.map(g=>`<label class="flex items-center gap-2"><input type="checkbox" data-disease="${g}" class="w-3.5 h-3.5 rounded accent-[var(--hive-red)]" ${(l.diseases||[]).includes(g)?"checked":""} disabled><span class="text-sm text-hive-text">${g}</span></label>`).join("")}</div>`)}
        ${J("pests","Pests",`<div class="grid grid-cols-2 gap-2 pb-2">${Ut.map(g=>`<label class="flex items-center gap-2"><input type="checkbox" data-pest="${g}" class="w-3.5 h-3.5 rounded accent-[var(--hive-red)]" ${(l.pests||[]).includes(g)?"checked":""} disabled><span class="text-sm text-hive-text">${g}</span></label>`).join("")}</div>`)}
      </section>
  ...
  `,Ie(e);let p=!0;const v=document.getElementById("lockToggle"),h=document.getElementById("lockIcon"),m=document.getElementById("lockLabel"),r=document.getElementById("saveBar"),u=e.querySelectorAll("input, textarea, select"),f=e.querySelectorAll(".temperament-pill");v.addEventListener("click",()=>{p=!p,u.forEach(g=>{g.id!=="weightTotal"&&(g.disabled=p)}),f.forEach(g=>g.disabled=p),p?(h.innerHTML='<path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z"/>',m.textContent="Locked",v.style.color="var(--hive-muted)",r.classList.add("hidden")):(h.innerHTML='<path stroke-linecap="round" stroke-linejoin="round" d="M13.5 10.5V6.75a4.5 4.5 0 119 0v3.75M3.75 21.75h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H3.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z"/>',m.textContent="Editing",v.style.color="var(--hive-gold)",r.classList.remove("hidden"))});const y=document.getElementById("strengthSlider"),w=document.getElementById("strengthValue");y.addEventListener("input",()=>{w.textContent=y.value+"%"}),f.forEach(g=>{g.addEventListener("click",()=>{p||(f.forEach(E=>E.className="temperament-pill btn-secondary flex-1 py-2 text-xs"),g.className="temperament-pill btn-primary flex-1 py-2 text-xs")})});const $=document.getElementById("weightLeft"),S=document.getElementById("weightRight"),I=document.getElementById("weightTotal"),N=()=>{const g=parseFloat($.value)||0,E=parseFloat(S.value)||0;(g>0||E>0)&&(I.value=(g+E).toFixed(2))};$.addEventListener("input",N),S.addEventListener("input",N),(M=document.getElementById("saveBtn"))==null||M.addEventListener("click",()=>{const g=document.createElement("div");g.className="fixed top-20 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 rounded-full text-sm font-medium animate-in",g.style.cssText="background:var(--hive-gold);color:#0B0D0E;box-shadow:0 4px 20px rgba(212,175,55,0.3)",g.textContent="Inspection updated",document.body.appendChild(g),setTimeout(()=>{g.style.opacity="0",setTimeout(()=>g.remove(),300)},2500)})}qe();Oe();A("#/login",kt);A("#/apiary",Lt);A("#/hive/:id",ne);A("#/apiary-dashboard",Mt);A("#/inspect",Dt);A("#/admin",qt);A("#/devices",jt);A("#/build/:id",Rt);A("#/hive-form/:id",Pt);A("#/tasks",Ot);A("#/notes",Ft);A("#/inspections",Wt);A("#/inspection/:id",Yt);window.addEventListener("hashchange",()=>setTimeout(we,50));We().then(()=>{De("#/login"),setTimeout(we,100)});
